...
export function InvoiceTableEntrada({ invoices, isLoading }: InvoiceTableProps) {
  ...
  const handleExportBook = () => {
    ...
    try {
      ...
      months.forEach(monthYear => {
        ...
        generateBookPDF(monthInvoices, { type: 'entrada', monthYear });
      });
  ...
  const handleExportToSheets = async () => {
    ...
    try {
      await sendToGoogleSheets(invoices, 'entrada', webhookUrl);
      toast({
              ...
              {isLoading ? (
                ...
                paginatedInvoices.map((invoice) => (
                  <TableRow key={invoice.id} className={cn(invoice.situacao === 'Cancelada' && 'text-muted-foreground')}>
                    ...
                    <TableCell>{invoice.emitente.nome}</TableCell>

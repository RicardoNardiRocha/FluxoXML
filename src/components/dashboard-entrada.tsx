...
export function DashboardEntrada() {
  ...
  const handleFileUpload = async (files: File[]) => {
    ...
    try {
      ...
      for (const file of files) {
        ...
        if (parsed.kind === "nfe") {
          if (parsed.meta.perspectiva !== "entrada") {
            if (parsed.meta.perspectiva === "saida") ignoredSaida++;
      ...
      if (newInvoices.length === 0 && files.length > 0 && cancelKeys.length === 0) {
        throw new Error("Nenhuma nota válida encontrada para o Livro de Entrada (destinatário não bate com a lista).");
      }
      ...
      toast({
        ...
        description: `Arquivos: ${files.length}. Ignoradas: ${ignoredSaida} (saída) e ${ignoredTerceiro} (terceiros).`,
        variant: 'success',
      });
  ...
  const filteredInvoices = useMemo(() => {
    return invoices.filter((invoice) => {
      ...
      const clientMatch = client ? invoice.emitente.nome.toLowerCase().includes(client.toLowerCase()) : true;
